...
class VoiceManager: NSObject, ObservableObject {
    ...
    func startSession() {
        ...
        guard let agentType = currentAgentType else {
            print("❌ No agent type set")
            return
        }
        
        print("🎤 Starting session for \(agentType.displayName)")
        isSessionActive = true
        
        // Initialize ElevenLabs session
        do {
            try ElevenLabsSDK.shared.initialize(agentId: agentType.agentId)
            status = .connected
            print("✅ ElevenLabs session initialized")
        } catch {
            print("❌ Failed to initialize ElevenLabs: \(error)")
    ...
    func endElevenLabsSession() {
        ...
        ElevenLabsSDK.shared.terminate()
